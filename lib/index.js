...
class MenuItem extends React__default["default"].Component {
  ...
  constructor(props) {
    ...
    _defineProperty(this, "handleItemHover", (key, relativeDepth = 0) => {
      this.clearHoverTimeout(); // disable menu closing if a child has been hovered over
  ...
  componentDidUpdate() {
    ...
    if (this.ref.current) {
      ...
      if (maxX > window.innerWidth) {
        this.ref.current.parentNode.getBoundingClientRect(); // top level

        if (this.ref.current.classList.contains('electronbar-top-menu-item-children')) ; else {
          console.log(this.ref.current.parentNode);
  ...
  render() {
    ...
    if (hasChildren) {
      ...
      if (hoisted) {
        itemContainer = items; // check it top level
...
class Electronbar {
  ...
  render(titleBarNeedsUpdate = false) {
    if (this.mountNode && this.browserWindow) {
      let contextMenu = this.contextMenu && this.contextMenuEvent && /*#__PURE__*/React__default["default"].createElement(Menu, {
        context: true,
      ...
      this.mountNode.render( /*#__PURE__*/React__default["default"].createElement(React__default["default"].Fragment, null, /*#__PURE__*/React__default["default"].createElement(TitleBar, {
  ...
  mount(mountNode) {
    if (mountNode) {
      this.unmount();
      this.mountNode = ReactDomClient__default["default"].createRoot(mountNode);
    }
  }
