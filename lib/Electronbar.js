	...
	(0, _createClass3.default)(TitleBar, [{
		...
		value: function render() {
			return _react2.default.createElement(
				...
				_react2.default.createElement(Buttons, { window: this.props.window })
			);
		}
	}]);
	return TitleBar;
...
var Menu = function (_React$Component2) {
	...
	function Menu(props) {
		...
		var _this2 = (0, _possibleConstructorReturn3.default)(this, (Menu.__proto__ || (0, _getPrototypeOf2.default)(Menu)).call(this, props));

		_this2.state = {
			selectedItemKey: null
		};

		_this2.handleWindowClick = function () {
			_this2.close();
		};

		_this2.handleItemClick = function (key) {
			_this2.setState({
				selectedItemKey: _this2.state.selectedItemKey == key ? null : key
			});
		};

		_this2.handleItemHover = function (key) {
			if (_this2.state.selectedItemKey != null) {
				_this2.setState({
					selectedItemKey: key
				});
			}
		...
		window.addEventListener('click', _this2.handleWindowClick, false);
		window.addEventListener('touchstart', _this2.handleWindowClick, false);
		return _this2;
	}
...
var MenuItem = function (_React$Component3) {
	...
	function MenuItem(props) {
		...
		var _this4 = (0, _possibleConstructorReturn3.default)(this, (MenuItem.__proto__ || (0, _getPrototypeOf2.default)(MenuItem)).call(this, props));

		_this4.hoverTimer = null;
		_this4.state = {
			selectedItemKey: null
		};
		...
		_this4.handleClick = function (e) {
			...
			if (!_this4.props.item.enabled) {
				return;
			}

			if (_this4.props.item.type == 'normal') {

				// set timeout so that React has a chance to update before any potemtially locking function is called.
				setTimeout(function () {
					return _this4.props.item.click();
				}, 50);

				_this4.close();
			} else if (_this4.props.onClick) {
				_this4.props.onClick(_this4.props.iKey);
			}
		};

		_this4.handleHover = function (e) {
			e.stopPropagation();
			if (_this4.props.item.enabled && _this4.props.onHover) {
				_this4.props.onHover(_this4.props.iKey);
			}
		};

		_this4.handleAntiHover = function () {
			_this4.clearHoverTimeout();

			_this4.hoverTimer = setTimeout(function () {
				_this4.hoverTimer = null;
		...
		_this4.handleItemClick = function (key) {
			_this4.clearHoverTimeout();
			_this4.setState({
				selectedItemKey: key
			});
		...
		_this4.handleItemHover = function (key) {
			_this4.clearHoverTimeout();

			if (_this4.state.selectedItemKey != key) {
				_this4.hoverTimer = setTimeout(function () {
	...
	(0, _createClass3.default)(MenuItem, [{
		...
		key: 'clearHoverTimeout',
		value: function clearHoverTimeout() {
			if (this.hoverTimer) {
		...
		value: function render() {
			...
			if (hasChildren) {
				...
				if (open && enabled && items.length && disabledCount < item.submenu.length) {
					itemContainer = _react2.default.createElement(
						'div',
						{ className: (0, _classnames2.default)(depth ? 'electronbar-menu-item-children' : 'electronbar-top-menu-item-children') },
						items
					);
				}
			}
			...
			if (item.type == 'separator') {
				...
				return _react2.default.createElement(
					'div',
					{ className: _classnames2.default.apply(undefined, ['electronbar-menu-item'].concat(classes)), onMouseEnter: this.handleHover, onMouseLeave: this.handleAntiHover },
					_react2.default.createElement(
						'div',
						{ className: 'electronbar-menu-item-label', onClick: this.handleClick },
						_react2.default.createElement(
							'div',
				...
				return _react2.default.createElement(
					'div',
					{ className: _classnames2.default.apply(undefined, ['electronbar-top-menu-item'].concat(classes)), onMouseEnter: this.handleHover, onMouseLeave: this.handleAntiHover },
					_react2.default.createElement(
						'div',
						{ className: 'electronbar-top-menu-item-label', onClick: this.handleClick },
						translateRole(item)
					),
					itemContainer
				);
...
var Buttons = function (_React$Component4) {
	...
	function Buttons(props) {
		...
		var _this5 = (0, _possibleConstructorReturn3.default)(this, (Buttons.__proto__ || (0, _getPrototypeOf2.default)(Buttons)).call(this, props));
		...
		_this5.destroy = function () {
			['enter-full-screen', 'leave-full-screen', 'maximize', 'unmaximize'].forEach(function (event) {
				_this5.window.removeListener(event, _this5.onWindowChange);
			});

			_this5.window.removeListener('close', _this5.destroy);
		};

		_this5.onWindowChange = function () {
			_this5.setState({
				maximized: _this5.window.isMaximized(),
				fullScreen: _this5.window.isFullScreen()
			});
		};

		_this5.handleUnFullscreenClick = function () {
			_this5.window.setFullScreen(false);
		};

		_this5.handleMinimizeClick = function () {
			_this5.window.minimize();
		};

		_this5.handleMaximizeClick = function () {
			if (_this5.window.isMaximized()) {
				_this5.window.unmaximize();
			} else {
				_this5.window.maximize();
			}
		};

		_this5.handleCloseClick = function () {
			_this5.window.close();
		};

		_this5.window = props.window;
		_this5.state.maximized = _this5.window.isMaximized();
		_this5.state.fullScreen = _this5.window.isFullScreen();

		['enter-full-screen', 'leave-full-screen', 'maximize', 'unmaximize'].forEach(function (event) {
			_this5.window.on(event, _this5.onWindowChange);
		});

		_this5.window.on('close', _this5.destroy);
		return _this5;
	}
